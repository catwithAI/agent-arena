...
`ModelProviderSection.kind` 的 before-validator 接受新旧值，model 实例统一保存 canonical
值；旧 `lane.yaml` 无需修改。示例和新配置只写 canonical 名。对应既有测试更新为断言
canonical 值，并新增 legacy-input test。
...
## 16. Capture policy 与 redaction

### 16.1 Effective policy
...
### Phase 5：Sandbox transparent redirect spike
