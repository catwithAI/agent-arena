...
agent-lane 当前采集三类 agent 的事件、thinking 和 env tool trace，能够回答 agent
...
observability，可以作为第一种接入源和实现参考。但 agent-lane 不能把核心观测能力建立在
某个外部 gateway 的持续存在上：

- 用户可能直连 OpenAI、Anthropic、OpenRouter 或自建 provider；
- Codex Responses、MCP stdio、Env Attempt Server 等流量不一定经过该 gateway；
- 外部 gateway 的部署位置、可用性、schema 和版本不由 agent-lane 控制；
- agent-lane 必须能保存可重放、可比较、与 attempt 同生命周期的本地证据。

因此，本 spec 将通信观测定义为 **agent-lane 自有的基础能力**。外部 llm-gateway、
原生 CLI 日志、内置 HTTP 代理和 MCP wrapper 都只是可插拔 capture source，不能成为
agent-lane canonical 数据模型、存储、API 或前端的硬依赖。
...
- `docs/0708-roadmap.md` P0 网关层；
- `docs/specs/harbor-research.md`；
- `docs/codex-vllm-installation.md`。

## 核心实验问题

**在不依赖特定第三方 gateway 的前提下，agent-lane 能否为每个 attempt 建立一份完整、
...
以下能力属于 agent-lane，不得委托给某个外部 gateway 作为唯一实现：
...
- agent-lane 内置 HTTP capture：可控 HTTP/HTTPS endpoint。

任何 source 缺失或不可用时，agent-lane 仍必须能完成 attempt，并明确展示观测覆盖不足。
...
agent-lane HTTP capture ───┼─→ canonical logical calls / hops ─→ wire.jsonl
...
9. **沙盒透明重定向 egress 骨架**：Harbor 的 docker 环境用 sidecar（GOST
   ...
   TLS），但这套「容器网络命名空间内 redirect + sidecar」正是 agent-lane 沙盒内
   流量采集的插入点：把 sidecar 换成会记录/MITM 的代理即可，**不依赖被测 CLI
   遵守 proxy 环境变量**（见 R8.1）。

不直接照搬 Harbor 的具体 ATIF 文件格式作为 wire schema；agent-lane 可以在 design 阶段
决定 trajectory 是否兼容/映射 ATIF，但网络 hop、MCP frame 和 payload blob 仍使用
agent-lane 自有 wire schema。
...
### R1 agent-lane 自有 canonical 数据模型

agent-lane 必须定义独立于任何 provider/gateway 的 versioned wire schema。
...
| `lane-http` | base URL 可注入或 agent-lane 控制的 HTTP 流量 | agent-lane 自有代理能力 |
| `mcp-stdio` | CC/Codex ↔ MCP server JSON-RPC | agent-lane 自有 wrapper |
...
4. 主动可控的 HTTP 请求注入 `x-lane-attempt-id`；只有 agent-lane 自身是逐调用 caller，
...
9. 同模型对比或多模型对比中，CC/Codex 选择了 agent-lane 命名的第三方 provider 时，流量
   必须可按 attempt 接入 agent-lane 内建反代；`full` policy 保存协议原生 request/response
...
agent-lane 必须能从 wire evidence 生成调用级上下文画像：
...
agent-lane 必须提供自有 MCP stdio wrapper，不能依赖网络 gateway。
...
### R8 agent-lane 自有 HTTP capture

对于 base URL 可注入或由 agent-lane 控制的流量，agent-lane 必须具备不依赖外部
...
9. Codex 只有在 gateway 提供 Responses API 后才能使用该 source，不能用 endpoint
   redirect 冒充协议兼容；
10. 移除 connector 后，agent-lane 的 wire API、历史数据和前端必须继续工作。
...
3. gateway/source 负责真实上游 credential，agent-lane wire log 永不保存 credential；
...
9. **与现有 provider 配置对齐，不另起一套 schema**：本节的 protocol/auth 声明
   ...
   - 协议命名统一：现有枚举 `openai-chat | anthropic | vllm-responses` 与本文的
     `openai-chat-completions | anthropic-messages | openai-responses` 是同一组
     概念的两套名字，design 阶段必须二选一并给出迁移映射（`vllm-responses` 与
     `openai-responses` 指同一 wire protocol，不得并存两个值）；
   - 认证模式显式化：新增 `auth_mode: bearer | api-key` 字段取代「adapter 硬编码
     注入 `ANTHROPIC_AUTH_TOKEN`」的隐式约定，两种模式对应的 env 注入不得互相
     转写（对照 Harbor 把 `ANTHROPIC_AUTH_TOKEN` 转写进 `ANTHROPIC_API_KEY` 的
     反例，见 `docs/specs/harbor-research.md` §2.3）；
   - lane.yaml 既有配置必须能不改动地继续工作（新字段有默认值：
     `kind: anthropic` 默认 `auth_mode: bearer`，与当前 adapter 行为一致）。
...
- 关闭/移除 llm-gateway connector 后，native-event、MCP、agent-lane HTTP capture 和历史
  wire 页面仍正常工作。
...
- 不让外部 llm-gateway 成为 agent-lane 启动依赖；
...
这些是对应 source 的依赖，不是 agent-lane 基础能力整体的启动依赖。
...
5. **agent-lane HTTP capture**：先反代/SSE，后扩协议；沙盒内流量的透明重定向
   形态（R8.1）按需落地；
6. **分析与 UI**：context 曲线、时序、retry、compaction；
7. **Responses/HTTPS 扩展**：按实际需求增加，不阻塞基础层交付。

这个顺序允许 agent-lane 尽快利用现有 llm-gateway 产出价值，同时从第一天起保持数据模型、
生命周期和用户体验独立于它。
